...
export function componentFixtures(
  ...
  return [
    ...
    f(
      ...
      "Accent palette",
      "Foundations",
      "Button fills, readable accent text, and two supporting accents.",
      swatches([
        "accentFill",
        "accent",
        "accent2",
...
if(b.hasAttribute('data-open-dialog'))section.querySelector('dialog').showModal();
...
export function componentDocument(
  ...
  return `<!doctype html><html lang="en" data-theme="${mode}" class="${embedded ? "embedded" : ""}"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'self' file: 'unsafe-inline'; script-src 'unsafe-inline'; font-src data: https: http:; img-src data:; base-uri 'none'; form-action 'none'"/><title>${escapeHTML(theme.name)} — Component library</title>${tokenCSS ? `<style>${tokenCSS.replace(/<\/style/gi, "< /style")}</style>` : '<link rel="stylesheet" href="tokens.css"/>'}<style>${fixtureStyles}\n${(
    ["dark", "light"] as const
  )
